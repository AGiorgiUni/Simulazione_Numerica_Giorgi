...
{ 
  ...
  int nconf = 1;
    ...
    int n=220;  //equilibration of the system: moving 220 times to obtain the requested temperature ( 150 is obtained by cout walker[it])
    //OBS: the value oscillates more than liquid phase also using bigger n
    for(int i=0;i<n;i++){
        Move();
    }
...
// distance i-j in pbc
      ...
      if(dr < rcut)
      {
        ...
        wij=1.0/pow(dr,12) - 0.5/pow(dr,6); // Viriale
        v += vij;
        w += wij;
      }
    }          
  ...
  walker[ip]= rho*walker[it]+(48*w/(double)npart)/(3*vol); //pressure
...
//stima_pres = rho*stima_temp+blk_av[ip]/blk_norm/(3*vol); //Pressure
