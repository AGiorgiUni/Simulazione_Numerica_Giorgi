...
{ 
  ...
  int nconf = 1;
    ...
    int n=150;  //equilibration for NVE
    for(int i=0;i<n;i++){
        Move();
    }
...
//Prepare arrays for measurements
  ...
  ie = 3; //Total energy
    ip=4; //add pressure
  n_props = 5; //Number of observables +1
...
// distance ip-i in pbc
    ...
    vtail=8*pi*rho*(1/(3*pow(rcut,9))-1/(pow(rcut,3)))/3;
  return 4.0*ene+vtail;
}
...
// distance i-j in pbc
      ...
      if(dr < rcut)
      {
          ...
          wij=1.0/pow(dr,12) - 0.5/pow(dr,6); //adding Viriale
          v += vij;
          w += wij;
      }
        min_dist = box/2.0;
        bin_index = int(ng*(dr/min_dist)); // posizione nell'istogramma
  ...
  for (int i=0; i<npart; ++i) kin += 0.5 * (vx[i]*vx[i] + vy[i]*vy[i] + vz[i]*vz[i]);
    ...
    wtail=96*pi*rho*(1/(3*pow(rcut,9))-1/((pow(rcut,3))*2))/3;
    walker[ip]=rho*walker[it]+(48*(w+wtail)/(double)npart)/(3*vol);
  return;
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    double gdr_norm=0;
    double dr =  (min_dist)/ng;
       for(int i=0; i<ng; i++)         // normalization
       {
         double r = i*dr;
